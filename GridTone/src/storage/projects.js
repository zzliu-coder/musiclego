...
(function (G) {
 'use strict';
 const DB_NAME='gridtone-local-v1', DB_VERSION=3;
 const recoverySummary=row=>({id:row.id,projectId:row.projectId,time:row.time,title:row.title,bytes:JSON.stringify(row.document).length*2});
 ...
 class ProjectRepository {
  ...
  async open(){
   ...
   if(!this.opening)this.opening=new Promise((resolve,reject)=>{
    ...
    r.onupgradeneeded=()=>{
     for(const name of ['projects','documents','project-index','recoveries','metadata','recovery-index'])if(!r.result.objectStoreNames.contains(name))r.result.createObjectStore(name);
     // One-time v2 backfill, atomic with the schema upgrade. Ordinary saves only
     // read this lightweight index, never the historical document bodies.
     const cursor=r.transaction.objectStore('recoveries').openCursor();
     cursor.onsuccess=()=>{const c=cursor.result;if(c){r.transaction.objectStore('recovery-index').put(recoverySummary(c.value),c.key);c.continue();}};
    };
  ...
  async migrate(){
   ...
   return new Promise((resolve,reject)=>{
    const tx=db.transaction(['projects','documents','project-index','recoveries','recovery-index','metadata'],'readwrite'),meta=tx.objectStore('metadata'),check=meta.get('migration-v2');
    const warnings=[];
    check.onsuccess=()=>{
     if(check.result)return;
     const request=tx.objectStore('projects').openCursor();
     request.onsuccess=()=>{const cursor=request.result;if(!cursor){
      const records=tx.objectStore('recoveries'),history=tx.objectStore('recovery-index'),all=history.getAll();
      all.onsuccess=()=>{const counts=new Map();let bytes=0;for(const row of all.result.sort((a,b)=>b.time-a.time)){const n=(counts.get(row.projectId)||0)+1;counts.set(row.projectId,n);if(n>10||bytes+row.bytes>120*1024*1024){records.delete(row.id);history.delete(row.id);}else bytes+=row.bytes;}meta.put({date:Date.now(),warnings},'migration-v2');};return;}
      ...
      if(key==='autosave'||key.startsWith('recovery:')){
       try{
        ...
        }else{const row={id:document.id+':legacy:'+key,projectId:document.id,time,title:document.title,document};tx.objectStore('recoveries').put(row,row.id);tx.objectStore('recovery-index').put(recoverySummary(row),row.id);}
  ...
  save(project){
   ...
   let copy;try{copy=G.validateProject(project);}catch(e){return Promise.reject(e);}
  ...
  async write(document,expected){
   ...
   return new Promise((resolve,reject)=>{
    const tx=db.transaction(['documents','project-index','recoveries','recovery-index'],'readwrite'),docs=tx.objectStore('documents'),index=tx.objectStore('project-index'),recoveries=tx.objectStore('recoveries'),history=tx.objectStore('recovery-index');
    ...
    request.onsuccess=()=>{
     ...
     usage.onsuccess=()=>{
      ...
      const prior=history.getAll();prior.onsuccess=()=>{
       const own=prior.result.filter(x=>x.projectId===id).sort((a,b)=>b.time-a.time);
       const added=!own.length||time-own[0].time>=30000;
       if(!added)return;
       const key=id+':'+time+':'+revision;recoveries.put({id:key,projectId:id,title:document.title,time,document},key);
       history.put({id:key,projectId:id,title:document.title,time,bytes:item.bytes},key);
       const discard=key=>{recoveries.delete(key);history.delete(key);};
       const obsolete=new Set(own.slice(9).map(r=>r.id));for(const key of obsolete)discard(key);
       // Recovery history has its own bounded budget. Never prune live documents.
       let bytes=item.bytes;
       for(const old of prior.result.filter(r=>!obsolete.has(r.id)).sort((a,b)=>b.time-a.time)){if(bytes+old.bytes>120*1024*1024)discard(old.id);else bytes+=old.bytes;}
      };
     };
    };
  ...
  async recoveries(id){return (await this.all('recovery-index')).filter(x=>x.projectId===id).sort((a,b)=>b.time-a.time).map(({id,time,title,projectId})=>({id,time,title,projectId}));}
  ...
  async delete(id){await this.queue.catch(()=>{});const db=await this.open();return new Promise((resolve,reject)=>{const tx=db.transaction(['documents','project-index','recoveries','recovery-index','metadata'],'readwrite');tx.objectStore('documents').delete(id);tx.objectStore('project-index').delete(id);const r=tx.objectStore('recovery-index').openCursor();r.onsuccess=()=>{const c=r.result;if(c){if(c.value.projectId===id){tx.objectStore('recoveries').delete(c.key);c.delete();}c.continue();}};const last=tx.objectStore('metadata').get('last-opened');last.onsuccess=()=>{if(last.result===id)tx.objectStore('metadata').delete('last-opened');};tx.oncomplete=()=>{this.revisions.delete(id);resolve();};tx.onabort=tx.onerror=()=>reject(tx.error);});}
 }
