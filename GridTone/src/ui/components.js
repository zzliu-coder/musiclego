...
(function (G) {
    ...
    const ib = (action, label, ic, attrs = '', variant = 'quiet') => button(action, '', ic, variant === 'secondary' ? 'soft-btn icon-btn' : 'icon-btn', `title="${label}" aria-label="${label}" ${attrs}`);
    ...
    // Semantic modules are presentation-only. Controllers supply trusted markup.
    G.ui.ActionTray=({label,body,kind='actions'})=>`<div class="action-tray" data-component="action-tray" data-kind="${esc(kind)}" role="group" aria-label="${esc(label)}">${body}</div>`;
    G.ui.FieldGroup=({title,body,note=''})=>`<section class="parameter-group" data-component="field-group"><h3>${esc(title)}</h3>${body}${note?`<p class="scope-note">${esc(note)}</p>`:''}</section>`;
    G.ui.PanelFrame=({header='',body='',actions=''})=>`${header}<div class="tool-panel-scroll" data-panel-slot="body">${body}</div>${actions?`<footer class="tool-panel-actions" data-panel-slot="actions">${actions}</footer>`:''}`;
    // body and drawing accept only markup assembled by production view code.
    G.ui.Dialog = ({title,body,subtitle=''}) => {const slots=typeof body==='string'?{body}:body;return `<div class="modal-backdrop" data-action="close-modal"></div><section class="modal" data-dialog-kind="${['form','confirm','menu'].includes(slots.kind)?slots.kind:'standard'}" role="dialog" aria-modal="true" aria-labelledby="modal-title"><header><div><h2 id="modal-title" tabindex="-1">${esc(title)}</h2>${subtitle?`<p>${esc(subtitle)}</p>`:''}</div>${G.ui.IconButton({action:'close-modal',label:'关闭',icon:'close',variant:'quiet'})}</header><div class="modal-body">${slots.body}</div>${slots.actions?`<footer class="modal-actions modal-fixed-actions" data-dialog-actions>${slots.actions}</footer>`:''}</section>`;};
