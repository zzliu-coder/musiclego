...
(function (G) {
    ...
    const button = (action, label, ic = '', cls = '', attrs = '') => {
        ...
        const size=cls.includes('icon-btn')?'icon':'normal';
        return `<button type="button" class="btn ${cls}" data-variant="${variant}" data-size="${size}" data-action="${action}" ${attrs}>${ic ? icon(ic) : ''}${label ? `<span>${label}</span>` : ''}</button>`;
    };
    ...
    G.ui.SegmentedControl = ({label,action,items,value,attribute='value'}) => {
        ...
        return `<nav class="segmented-control" aria-label="${esc(label)}">${items.map(([id,text])=>button(esc(action),esc(text),'',id===value?'active':'quiet',`data-${attribute}="${esc(id)}" aria-pressed="${id===value}"`)).join('')}</nav>`;
    };
    ...
    G.ui.CandidateBar = ({count,chosen,drawing='',actions=''}) => `<section class="candidate-bar">${G.ui.SegmentedControl({label:'候选版本',action:'creation-choose',attribute:'index',value:chosen,items:Array.from({length:count},(_,i)=>[i,'方案 '+(i+1)])})}<div class="candidate-preview">${drawing}</div><div class="modal-actions">${actions}</div></section>`;
  G.ui.TrackHeader=function({track,index=0,selected=false,solo=false,density='standard',soundName='',trackIcon='melody'}){
    ...
    return `<div class="arrange-track-head ${selected?'current':''}" data-density="${density}"><div class="track-title-row"><span class="track-number">${index+1}</span><button class="arrange-track-name" data-action="track" data-id="${t.id}" title="${esc(t.name)} · ${esc(soundName)}"><span>${icon(trackIcon,20)}</span><div><strong>${esc(t.name)}</strong><small>${esc(soundName)}</small></div></button>${ib('track-options','音轨设置','dots',`data-id="${t.id}"`)}</div><div class="track-controls">${button('mute','M','',t.mute?'active':'quiet',`data-id="${t.id}" aria-label="静音 ${esc(t.name)}" aria-pressed="${t.mute}"`)}${button('solo','S','',solo?'active':'quiet',`data-id="${t.id}" aria-label="只听 ${esc(t.name)}" aria-pressed="${solo}"`)}<input type="range" min="0" max="1" step=".01" value="${t.volume}" data-range="track.volume" data-track="${t.id}" aria-label="${esc(t.name)} 音量"></div></div>`;
  };
})(globalThis.GridTone ||= {});
