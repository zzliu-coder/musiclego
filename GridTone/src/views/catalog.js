...
(function (G) {
    ...
    class CatalogUI {
        ...
        items(){const p=this.c.getProject(),all=this.category==='templates'?G.catalogTemplates():this.category==='drumkits'?G.projectKits(p):G.projectPresets(p);const q=this.search.trim().toLowerCase();return all.filter(x=>(this.category!=='templates'||this.role==='all'||this.roleOf(x)===this.role)&&(!q||[x.name,x.description,x.category,...(x.tags||[])].join(' ').toLowerCase().includes(q)));}
        cards(){const esc=this.c.esc,category=this.category,roleNames={drums:'鼓点',melody:'旋律',bass:'贝斯',chords:'和弦',song:'整曲起点'};return this.items().map(x=>{
            ...
            return G.ui.TemplateCard({id:x.id,kind:category,title:x.name,summary:type+(x.bpm?' · '+x.bpm+' BPM':''),description:category==='drumkits'?[...new Set(x.rows.map(r=>r.name||r.role))].slice(0,4).join(' · '):x.description||'可配置的乐器与鼓件集合。',drawing});
        ...
        resourceManager() {
            const {button,esc,openModal}=this.c,packs=G.catalogContents().packs,visible=packs.filter(p=>!G.BUNDLED_CATALOG_IDS?.includes(p.id));
            this.managerMode=true;this.c.playback.endAudition();
            openModal('素材管理与导入',`<p>模板在音乐模板库挑选，音色从本轨选择器使用。这里统一管理导入与备份。</p><div class="button-row">${button('import-catalog','导入素材包','upload','soft-btn')}${button('catalog-tab','管理鼓组','drum','quiet','data-category="drumkits"')}${button('example-catalog','下载示例素材包','download','quiet')}</div><div class="project-list">${visible.map(p=>`<article><strong>${esc(p.name)}</strong><p>${p.templates.length} 模板 · ${p.presets.length} 音色 · ${p.drumkits.length} 鼓组</p><small>${esc(p.id)}</small></article>`).join('')||'<p>暂未导入额外素材包。内置声音和模板已随应用保存。</p>'}</div><footer class="catalog-footer"><span>已导入 ${G.catalogPackCount()} 个素材包</span>${packs.length?button('export-catalog','备份素材库','save','soft-btn'):''}${button('close-modal','完成','','quiet')}</footer>`);
        }
        listing(category = this.category) {
            this.category = category;this.selected = null;this.frozen=null;this.c.playback.endAudition();
            const { button, esc, openModal } = this.c, packCount = G.catalogPackCount();
            openModal('模板与素材', `<div class="catalog-top"><nav class="catalog-tabs">${(this.managerMode?[['drumkits','鼓组']]:[['templates', '音乐模板'], ['presets', '音色预设'], ['drumkits', '鼓组']]).map(([k, n]) => button('catalog-tab', n, '', this.category === k ? 'active' : 'quiet', `data-category="${k}" aria-pressed="${this.category===k}"`)).join('')}</nav>${this.managerMode?button('catalog','返回资源管理','folder','quiet'):button('recipes','组合模板','spark','soft-btn')}${button('import-catalog', '导入素材包', 'upload', 'soft-btn')}</div><p class="modal-copy">${category==='drumkits'?'选择鼓组后指定音轨；更换影响整轨，原音符保持。':'先听，再应用。内容会展开为普通音符；试听保持当前作品不变。'}</p>${category==='templates'?`<nav class="catalog-role-tabs" aria-label="模板角色">${[['all','全部'],['song','整曲起点'],['drums','鼓点'],['chords','和弦'],['melody','旋律'],['bass','贝斯']].map(([id,l])=>button('catalog-role',l,'',this.role===id?'active':'quiet',`data-role="${id}" aria-pressed="${this.role===id}"`)).join('')}</nav>`:''}<div class="catalog-filter-line"><input type="search" data-field="catalog-search" aria-label="搜索素材" placeholder="搜索名称、风格或用途" value="${esc(this.search)}"><small id="catalog-count">${this.items().length} 项</small></div><div class="catalog-grid" id="catalog-cards">${this.cards()}</div><div class="catalog-footer"><span>已导入 ${packCount} 个素材包</span>${button('example-catalog', '示例素材包', 'download', 'quiet')}${packCount ? button('export-catalog', '备份素材库', 'save', 'quiet') : ''}</div>`);
        }
        select(kind, id) {
            this.frozen=null;
            if (kind === 'templates') {
            ...
            openModal('使用' + (kind === 'drumkits' ? '鼓组' : '音色'), {body:`<h3>${esc(object.name)}</h3><p class="modal-copy">影响所选音轨全部音乐块。新音轨会附带一个可编辑的试听音乐块。</p><label class="form-label">目标音轨<select data-field="catalog-track">${allowed.map(t => `<option value="${t.id}" ${t.id === this.options.trackId ? 'selected' : ''}>${esc(t.name)}</option>`).join('')}<option value="new-track" ${this.options.trackId === 'new-track' ? 'selected' : ''}>新增音轨</option></select></label>`,actions:`${button('catalog-back', '返回素材库', '', 'quiet')}${button('catalog-preview', '试听结果', 'headphones', 'soft-btn')}${button('catalog-apply', '确认使用', 'check', 'dark-btn')}`});
        }
        detail() {
            ...
            openModal(t.name, {body:`<p class="modal-copy">${esc(t.description)}</p><div class="template-summary"><span>${song ? '完整曲目 · ' + t.project.tracks.length + ' 轨 / ' + t.project.bars + ' 小节' : (t.kind === 'drum' ? '鼓点' : '音符') + '模板 · ' + t.bars + ' 小节'}</span><span>${song ? '速度 ' + t.project.bpm + ' BPM' : G.KEYS[t.key] + ' · ' + ({ major: '大调', minor: '小调', pentatonic: '五声音阶', chromatic: '半音阶' }[t.scale])}</span></div>
   ${song ? '<p class="resource-warning">确认使用将保存当前作品，并新建一份模板作品；可在“我的作品”切回。试听保持当前作品不变。</p>' : `<div class="form-grid"><label class="form-label">应用方式<select data-field="catalog-mode">${[['new', '新增独立音乐块'], ['replace', '替换当前音乐块的内容'], ['new-track', '新增音轨和音乐块']].map(([v, l]) => `<option value="${v}" ${o.mode === v ? 'selected' : ''}>${l}</option>`).join('')}</select></label>${o.mode !== 'new-track' ? `<label class="form-label">目标音轨<select data-field="catalog-track">${tracks.map(tr => `<option value="${tr.id}" ${tr.id === o.trackId ? 'selected' : ''}>${esc(tr.name)}</option>`).join('')}</select></label>` : ''}
   ${o.mode === 'replace' ? `<label class="form-label">替换音乐块<select data-field="catalog-pattern">${(target?.patterns || []).map(pat => `<option value="${pat.id}" ${pat.id === o.patternId ? 'selected' : ''}>${esc(pat.name)} · ${pat.bars} 小节</option>`).join('')}</select></label>` : `<label class="form-label">插入第几小节<input type="number" data-field="catalog-bar" min="1" max="256" step="1" value="${o.bar + 1}"></label>`}
   ...
   ${o.mode !== 'new-track' ? `<label class="catalog-check"><input type="checkbox" data-field="catalog-sound" ${o.applySound ? 'checked' : ''}> 同时换成本模板建议的音色${t.kind === 'drum' ? '与鼓组' : ''}（影响本轨全部音乐块）</label>` : ''}
   ${o.mode === 'replace' ? `<p class="resource-warning">将替换 ${esc(pat?.name || '所选音乐块')}；${refs} 处共享引用同步变化。较短模板会重复填满现有音乐块。所有修改可以一次撤销。</p>` : '<p class="modal-copy">新增音乐块拥有独立编号，后续修改不会影响原素材。遇到已占用位置会阻止应用。</p>'}`}
   <p id="catalog-feedback" class="resource-feedback" role="status"></p>`,actions:`${button('catalog-back', '返回素材库', '', 'quiet')}${button('catalog-preview', '试听结果', 'headphones', 'soft-btn')}${button('stop', '停止试听', 'stop', 'quiet')}${button('catalog-apply', song ? '新建模板作品' : '确认应用', 'check', 'dark-btn')}`});
        }
        candidate() {
        ...
        buildCandidate() {
            ...
            if (t.kind === 'drumkits') {
                G.pinKit(q, t.id);
                if(this.options.trackId!=='new-track'){
                    const sourceRows=G.drumsFor(q,tr);
                    // Keep each hit's role and all timing/IDs when key assignments differ.
                    for(const pat of tr.patterns)pat.notes=G.remapDrums(pat.notes,sourceRows,resource.rows);
                }
                tr.drumkitId = t.id;
                if (this.options.trackId === 'new-track')
        ...
        async preview() { try {
            ...
            const started=await this.c.playback.audition(result.project, scope, '素材试听 · ' + this.selected.name);
            if(started)this.feedback('正在试听方案，当前作品保持不变。');
        }
        catch (e) {
        ...
        async importFile(file) {
            try {
                ...
                this.c.toast(message + (persistent ? '' : ' 本机素材库存储不可用，请保存原素材包；已使用的声音会随作品文件保存。'));
            }
            catch (e) {
        ...
        handleAction(action, el) {
            ...
            if(action==='catalog'){this.role=el?.dataset.role||'all';this.search='';this.resourceManager();return true;}
            try {
                if (action === 'catalog-tab') {
                    if(el.dataset.category==='templates'&&this.c.openLibrary){this.c.closeModal();this.c.openLibrary('all');}else if(el.dataset.category==='presets'&&this.c.openLibrary){this.c.closeModal();this.c.openLibrary('sound');}else this.listing(el.dataset.category);
                    return true;
                }
                ...
                if (action === 'example-catalog') {
                    G.downloadBlob(new Blob([JSON.stringify(G.EXAMPLE_CATALOG, null, 2)], { type: 'application/json' }), '乐构_示例素材包.gridtonepack');
                    return true;
                }
                if (action === 'export-catalog') {
                    ...
                    G.downloadBlob(new Blob([JSON.stringify(combined, null, 2)], { type: 'application/json' }), '乐构_素材库备份.gridtonepack');
                    return true;
                }
            }
            catch (e) {
