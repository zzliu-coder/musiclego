...
(function (G) {
    ...
    class CatalogUI {
        ...
        cards(){const esc=this.c.esc,category=this.category,roleNames={drums:'鼓点',melody:'旋律',bass:'贝斯',chords:'和弦',song:'整曲起点'};return this.items().map(x=>{
            ...
            return G.ui.TemplateCard({id:x.id,kind:category,title:x.name,summary:type+(x.bpm?' · '+x.bpm+' BPM':''),description:x.description||'可配置的乐器与鼓件集合。',drawing});
        ...
        select(kind, id) {
            ...
            openModal('使用' + (kind === 'drumkits' ? '鼓组' : '音色'), `<h3>${esc(object.name)}</h3><p class="modal-copy">影响所选音轨全部音乐块。新音轨会附带一个可编辑的试听音乐块。</p><label class="form-label">目标音轨<select data-field="catalog-track">${allowed.map(t => `<option value="${t.id}" ${t.id === this.options.trackId ? 'selected' : ''}>${esc(t.name)}</option>`).join('')}<option value="new-track" ${this.options.trackId === 'new-track' ? 'selected' : ''}>新增音轨</option></select></label><div class="modal-actions">${button('catalog-back', '返回素材库', '', 'quiet')}${button('catalog-preview', '试听结果', 'headphones', 'soft-btn')}${button('catalog-apply', '确认使用', 'check', 'dark-btn')}</div>`);
        }
        detail() {
   ...
   <p id="catalog-feedback" class="resource-feedback" role="status"></p><div class="modal-actions">${button('catalog-back', '返回素材库', '', 'quiet')}${button('catalog-preview', '试听结果', 'headphones', 'soft-btn')}${button('stop', '停止试听', 'stop', 'quiet')}${button('catalog-apply', song ? '新建模板作品' : '确认应用', 'check', 'dark-btn')}</div>`);
        }
        candidate() {
