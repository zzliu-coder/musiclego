(function(G){'use strict';
 class TemplateShelf{
  ...
  all(){const key=this.c.getProject().key+':'+G.catalogRevision();if(this.cache?.key===key)return this.cache.items;const items=[...G.catalogTemplates().map(t=>t.type==='song'?{...t,bars:t.project.bars}:t),...[...G.RECIPES,...(G.STUDIO_COMBOS||[])].map(r=>({...r,type:'recipe',role:'song',bars:r.bars|| (r.answer?8:(G.PROGRESSIONS.find(p=>p.id==='progression.'+r.progression)?.bars||4))})),...G.PROGRESSIONS.map(r=>G.generateProgression(r.id,{key:this.c.getProject().key}).template),...G.PROGRESSIONS.map(r=>{const texture=G.generateProgression(r.id,{key:this.c.getProject().key,rhythm:'arp'}).template;return {...texture,id:texture.id+'.texture',name:r.name+' · 分解伴奏',role:'texture'};})];this.cache={key,items};return items;}
  ...
  install(){
   ...
   document.addEventListener('dragover',e=>{
    if(!this.ownsDrag(e))return;const target=this.location(e);if(!target){this.clearGhost();this.ghostKey=null;e.dataTransfer.dropEffect='none';return;}e.preventDefault();
    ...
    if(target.svg){const g=document.createElementNS('http://www.w3.org/2000/svg','g');g.setAttribute('class','shelf-drop-ghost');g.setAttribute('pointer-events','none');g.style.pointerEvents='none';g.dataset.valid=String(valid);const h=Number(target.svg.getAttribute('height'))-102,w=m.bars*16*target.cw;
     g.innerHTML=`<rect x="${target.x}" y="28" width="${w}" height="${h}" rx="5" fill="var(--selected)" fill-opacity=".65" stroke="${valid?'var(--brand)':'var(--danger)'}" stroke-width="2" stroke-dasharray="6 4"/><foreignObject x="${target.x+4}" y="33" width="${Math.max(40,w-8)}" height="${Math.min(110,h)}"><div xmlns="http://www.w3.org/1999/xhtml">${G.ui.musicBlock.projection(m,label,valid)}</div></foreignObject>`;target.svg.insertBefore(g,target.svg.querySelector('#pitch-axis'));return;}
