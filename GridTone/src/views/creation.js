 ...
 class CreationUI{
  ...
  render(){
   ...
   if(candidate){
    ...
    if(x.mode==='mix'){drawing='<div class="mix-candidate-changes">'+candidate.project.tracks.map(tr=>{const old=x.base.tracks.find(v=>v.id===tr.id);return old&&(old.volume!==tr.volume||old.pan!==tr.pan)?`<p><b>${esc(tr.name)}</b> · 音量 ${F.percent(old.volume)} → ${F.percent(tr.volume)}<br>左右位置 ${F.pan(old.pan)} → ${F.pan(tr.pan)}</p>`:'';}).join('')+'</div>';stats='仅改变音量和左右位置';}
    results=`<section class="candidate-bar" data-component="candidate" data-phase="compare"><header><strong>挑一版，听听看</strong><small>尚未写入作品</small></header>${U.SegmentedControl({label:'方案版本',action:'creation-choose',attribute:'index',value:x.chosen,items:x.candidates.map((_,i)=>[i,'方案 '+(i+1)])})}<div class="candidate-preview">${drawing}</div><p class="candidate-stats">${stats}</p>${U.PreviewControls({actions:button('creation-preview','连伴奏听','headphones','soft-btn')+button('creation-preview-solo','只听这一轨','play','quiet small-btn')+button('creation-preview-original','听原稿','','quiet small-btn')+button('stop','停止','stop','quiet small-btn'),context:'方案和原稿使用相同的音乐范围。'})}${instance?`<div class="candidate-preview-mode"><span>画板对照</span>${button('creation-overlay','方案','layers','small-btn '+(x.overlay==='original'?'quiet':'active'),'data-overlay="candidate"')}${button('creation-overlay','原稿','','small-btn '+(x.overlay==='original'?'active':'quiet'),'data-overlay="original"')}${button('creation-fit','看全方案','grid','quiet small-btn')}</div>`:''}<div class="candidate-commit">${button('creation-apply','用这版','check','dark-btn',wrongTarget?'disabled title="请先回到原来的音乐块"':'')}${button('creation-generate',op.random?'再来一组':'重新计算','spark','quiet')}</div></section>`;
   }
   ...
   const body=`<div class="creation-target-contract" data-phase="${candidate?'compare':x.flow?.state||'prepare'}">${context}${quickKeep}${status}${results}${candidate?`<details id="creation-tuning" ${captureOpen('creation-tuning')?'open':''}><summary>调整方法与参数</summary>${setup}</details>`:setup+blocked+`<div class="creation-generate-row">${button('creation-generate',op.random?'做几版':'准备方案','spark','dark-btn',prerequisite?'disabled title="先补齐上方材料"':'')}</div>`}<p id="creation-feedback" role="status"></p>${advanced?`<details id="creation-options" ${captureOpen('creation-options')?'open':''}><summary>范围与细节${instance&&x.mode!=='drums'?' · '+G.noteName(x.low)+'—'+G.noteName(x.high):''}</summary>${advanced}</details>`:''}${keep}${insights}</div>`;
   (this.c.openCreation||this.c.openModal)(LABELS[x.mode],body,'基于现有音乐做变化 · 采用前原稿保持');this.c.updateProjection?.();
  }
  generate(){
