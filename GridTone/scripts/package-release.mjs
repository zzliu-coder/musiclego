...
const evidence=root+(process.env.EVIDENCE_DIR||'docs/closure-v2/evidence/current')+'/';
...
try{
 ...
 await writeFile(evidence+'package.json',JSON.stringify(receipt,null,2));await writeFile(evidence+'package-build.txt',log);await writeFile(archive+'.sha256',receipt.archive_sha256+'  '+basename(archive)+'\n');console.log(receipt);
 await rm(temp,{recursive:true,force:true});
}catch(e){await writeFile(evidence+'package-build.txt',log+'\n'+e.stack);throw e;}
