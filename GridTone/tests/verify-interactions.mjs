...
try{
 ...
 await check('R3-T02/T09 bounded components and one semantic token update reach production controls',async()=>{
  await click('catalog');await click('recipes');const before=await p.locator('[data-action="creation-generate"]').evaluate(e=>getComputedStyle(e).backgroundColor);
  await p.evaluate(()=>document.documentElement.style.setProperty('--brand','#7a40ac'));await p.waitForFunction(()=>getComputedStyle(document.querySelector('[data-action="creation-generate"]')).backgroundColor==='rgb(122, 64, 172)');const after=await p.locator('[data-action="creation-generate"]').evaluate(e=>getComputedStyle(e).backgroundColor);assert.equal(after,'rgb(122, 64, 172)');assert.notEqual(after,before);await p.evaluate(()=>document.documentElement.style.removeProperty('--brand'));await click('creation-close');
