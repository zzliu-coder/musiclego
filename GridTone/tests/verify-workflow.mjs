...
const click=action=>page.locator(`button[data-action="${action}"]`).filter({visible:true}).first().click();
...
try{
 ...
 await check('R3-T01 templates and progressions immediately reachable',async()=>{await click('catalog');assert.ok(await page.getByText('模板与素材',{exact:true}).isVisible());await click('close-modal');await click('composer');assert.ok(await page.locator('.progression-list').isVisible());await click('close-modal');});
 await check('R5-T03/T07 recipe preview and atomic apply share identical candidate',async()=>{
  await click('creation');await page.locator('[data-field="creation-mode"]').selectOption('recipe');await click('creation-generate');const candidate=await page.evaluate(()=>GridToneApp.creation.session.candidates[0].project);const count=await page.evaluate(()=>GridToneApp.getHistory().undo);await click('creation-preview');await page.waitForFunction(()=>GridToneApp.engine.playing);await click('creation-apply');assert.deepEqual(await page.evaluate(()=>GridToneApp.getProject()),candidate);assert.equal(await page.evaluate(()=>GridToneApp.getHistory().undo),count+1);await click('stop');
 });
 await check('R6-T06/T09 melody candidates cancel without music or history writes',async()=>{
  const before=await page.evaluate(()=>({project:GridToneApp.getProject(),history:GridToneApp.getHistory()}));await click('creation');await click('creation-generate');assert.equal(await page.locator('[data-action="creation-choose"]').count(),3);await click('creation-preview');await page.waitForFunction(()=>GridToneApp.engine.playing);await click('close-modal');assert.deepEqual(await page.evaluate(()=>({project:GridToneApp.getProject(),history:GridToneApp.getHistory()})),before);
 });
 await check('R6-T03 retention persists and regeneration changes only unprotected fields',async()=>{
  await click('creation');await click('creation-generate');await click('creation-apply');await page.locator('#gridframe').focus();await page.keyboard.press('Meta+a');assert.ok((await page.evaluate(()=>GridToneApp.getState().selected)).length>0);await click('creation');await page.locator('[data-action="creation-keep"][data-keep="rhythm"]').click();await click('creation-save-keeps');const before=await page.evaluate(()=>{const A=GridToneApp,s=A.getState(),t=A.getProject().tracks.find(t=>t.id===s.trackId);return t.patterns.find(p=>p.id===s.patternId).notes.map(n=>[n.id,n.start,n.duration]);});await click('creation');await page.locator('[data-field="creation-mode"]').selectOption('rhythm');await click('creation-generate');await click('creation-apply');assert.deepEqual(await page.evaluate(()=>{const A=GridToneApp,s=A.getState(),t=A.getProject().tracks.find(t=>t.id===s.trackId);return t.patterns.find(p=>p.id===s.patternId).notes.map(n=>[n.id,n.start,n.duration]);}),before);
 });
 await check('R6-T10 stale candidate is rejected',async()=>{
  await click('creation');await click('creation-generate');await page.evaluate(()=>{GridToneApp.creation.session.token='outdated';});const before=await page.evaluate(()=>GridToneApp.getProject());await click('creation-apply');assert.deepEqual(await page.evaluate(()=>GridToneApp.getProject()),before);assert.match(await page.locator('#creation-feedback').innerText(),/已变化/);await click('close-modal');
 });
