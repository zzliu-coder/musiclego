"""1.8 migration of the 16 second-pass behavioral checks. Changed UI entry paths are documented; musical assertions remain. DOM and Web Audio real; document fixtures via public commit.
...
ap=argparse.ArgumentParser();ap.add_argument('--out',default='docs/workspace-1.8/evidence/regressions');ap.add_argument('--url');ap.add_argument('--browser',default=os.getenv('CHROMIUM_PATH','/usr/bin/chromium'));args=ap.parse_args()
...
with sync_playwright() as pw:
 ...
 def action(p,a):
  if a in ['composer','creation'] and not p.locator('button[data-action="'+a+'"]').filter(visible=True).count():
   p.locator('button[data-action="edit-tools"]').click()
  p.locator('button[data-action="'+a+'"]').filter(visible=True).first.click()
 ...
 def bake(p):
  ...
  action(p,'open-sound');p.locator('[data-panel="pipeline"]').click();action(p,'bake-pipeline');action(p,'confirm')
 ...
 def panel_switch(p):
  # 1.8 keeps inactive drafts, with exactly one visible side-panel. Each close targets its owner.
  action(p,'composer');action(p,'composer-preview');assert ev(p,'!!GridToneApp.composer.options')
  action(p,'edit-tools');action(p,'creation');assert ev(p,'!!GridToneApp.composer.options')
  action(p,'creation-generate');assert ev(p,'GridToneApp.creation.session.candidates.length')>0
  action(p,'edit-tools');action(p,'composer');assert ev(p,'!!GridToneApp.creation.session')
  action(p,'creation-close');assert ev(p,'GridToneApp.composer.options===null && !!GridToneApp.creation.session')
  action(p,'edit-tools');p.locator('[data-action="resume-tool"][data-panel="creation"]').click()
  action(p,'creation-close');assert ev(p,'GridToneApp.composer.options===null && GridToneApp.creation.session===null');assert ev(p,'document.querySelector("#creation-dock").hidden')
